...
const generateHostelRecord = async (count, total) => {
  const hostelArray = []; // define array for moving hostel review window
  while (count < total) { // iterate through all 10000000 hostel records
    try {
      ...
      {
        console.log('Reviews author committed to: ' + i + ' / ' + numberofReviews);
        const index = count % 30;
        if (hostelArray[index] === undefined) {
          hostelArray.push(index);
          hostelArray[index] = [];
        }
        hostelArray[index].push(currentAuthor);
      }
      for (let j = count; j < (count + numberofReviews); j++) {
        const reviewData = await generateReview();
        let hostel = 'hostel' + j;
        if (j > count) {
          hostel = 'hostel' + (j - count);
        }
        ...
        console.log('Primary Records: ' + count);
        console.log({hostelId: hostel, review: reviewData, user: currentAuthor, description: desc, age: age, gender: gender, image: img, security: security, location: location, staff: staff, atmosphere: atmosphere, cleanliness: cleanliness, facilities: facilities, value: value, total: total, date: date});
        await writer.write({hostelId: hostel, review: reviewData, user: currentAuthor, description: desc, age: age, gender: gender, image: img, security: security, location: location, staff: staff, atmosphere: atmosphere, cleanliness: cleanliness, facilities: facilities, value: value, total: total, date: date});
        totalRows += 1;
      }
      // currentAuthor++; //move on to the next author
    } catch (err) {
      console.log(err);
      process.exit();
    }
    await generateHostelRecord(count += 1);
  };

};

const timer = async (count, total) => {
  try {
    const startTime = new Date();
    await generateHostelRecord(count, total);
    writer.end();
